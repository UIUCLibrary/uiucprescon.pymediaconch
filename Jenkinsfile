                                                                        ...
                                                                        retry(2){
                                                                            ...
                                                                            withEnv([(attempt == 1) ? "UV_OFFLINE=1" : 'UV_OFFLINE=0']){
                                                                                sh(
                                                                                    ...
                                                                                    script: "uv run --frozen --only-group=tox-uv tox -e py${pythonVersion.replace('.', '')} --installpkg ${findFiles(glob:'dist/*.whl')[0].path} -vv"
                                                                                )
                                                                            }
                                                                        }
                                                                    }
        ...
        [
            "Python ${pythonVersion} - Mac":{
                stage("Python ${pythonVersion} - Mac"){
                    if(params.INCLUDE_MACOS != true){
                        Utils.markStageSkippedForConditional("Python ${pythonVersion} - Mac")
                    }
                    stage("Wheel for Python ${pythonVersion}"){
                        if(params.INCLUDE_MACOS != true){
                            Utils.markStageSkippedForConditional("Wheel for Python ${pythonVersion}")
                        }
                        stage('Build MacOS Universal2 Wheel'){
                            if(params.INCLUDE_MACOS != true){
                                Utils.markStageSkippedForConditional('Build MacOS Universal2 Wheel')
                            } else {
                        ...
                        stage('Test Universal2 Wheel'){
                            if(testPackages != true || params.INCLUDE_MACOS != true){
                                                ...
                                                retry(conditions: [agent()], count: 2) {
                                                    if(testPackages == true){
                                                        stage("Test Wheel (${pythonVersion} MacOS ${arch})"){
                                                            if(params.INCLUDE_MACOS != true){
                                                                Utils.markStageSkippedForConditional("Test Wheel (${pythonVersion} MacOS ${arch})")
                                                            } else {
                                                                retry(conditions: [agent()], count: 2) {
                                                                                    ...
                                                                                    withEnv(["TOX_UV_PATH=${WORKSPACE}/venv/bin/uv", "PYMEDIACONCH_SAMPLE_FILES=${WORKSPACE}/sample_files"]){
                                                                                        timeout(60){
                                                                                            sh(label: 'Running Tox',
                                                                                               ...
                                                                                               ./venv/bin/uv run --frozen --only-group=tox-uv tox run --installpkg ${it.path} -e py${pythonVersion.replace('.', '')} -vv"""
                                                                                            )
                                                                                        }
                                                                                    }
                                                                                }
...
def get_mac_abi3_wheel_stages(pythonVersionsTotestAbi3Wheels, testPackages, params, wheelStashes, retryTimes){
    ...
    buildStages['Python ABI3 wheel'] = {
        stage('Python ABI3 wheel') {
            if(params.INCLUDE_MACOS != true){
                Utils.markStageSkippedForConditional('Python ABI3 wheel')
            }
            stage('Building Python ABI3 wheel') {
                if(params.INCLUDE_MACOS == true){
                    ...
                    Utils.markStageSkippedForConditional('Building Python ABI3 wheel')
                }
            }
            stage('Testing Python ABI3 wheel') {
                if(testPackages != true || params.INCLUDE_MACOS != true){
                ...
                parallel([:] << allValidArches.collectEntries{arch ->
                    ...
                    pythonVersionsTotestAbi3Wheels.each{pythonVersion ->
                        ...
                        testingStages[newWheelStage] = {
                            if(testPackages == true && selectedArches.contains(arch) && params.INCLUDE_MACOS == true){
                                node("mac && python${pythonVersion} && ${arch}"){
                                    checkout scm
                                    try{
                                            ...
                                            withEnv(["TOX_UV_PATH=${WORKSPACE}/venv/bin/uv", "PYMEDIACONCH_SAMPLE_FILES=${WORKSPACE}/sample_files"]){
                                                timeout(60){
                                                    sh(label: 'Running Tox',
                                                       script: """python${pythonVersion} -m venv venv
                                                       ./venv/bin/python -m pip install --disable-pip-version-check uv
                                                       ./venv/bin/uv run --frozen --only-group=tox-uv tox --installpkg ${it.path} -e py${pythonVersion.replace('.', '')} -vv"""
                                                    )
                                                }
                                            }
                                        }
                                                                ...
                                                                image.inside("--label=purpose=ci --label \"absoluteUrl=${currentBuild.absoluteUrl}\" --label \"JOB_NAME=${env.JOB_NAME}\" --label \"BUILD_NUMBER=${currentBuild.number}\" --mount source=python-tmp-uiucpreson-pymediaconch,target=/tmp --tmpfs /cache:exec --tmpfs /.local/share:exec --tmpfs /.local/bin:exec -e TOX_WORK_DIR=/cache/tox -e UV_PROJECT_ENVIRONMENT=/cache/uv_project_environment") {
                                                                    withEnv([
                                                                        ...
                                                                        retry(retryTimes){
                                                                            try{
                                                                                sh( label: 'Running Tox',
                                                                                    script: "uv run --frozen --only-group=tox-uv --isolated --python-preference system tox run -e ${toxEnv} --runner uv-venv-lock-runner -vv"
                                                                                    )
                                                                            } catch(e){
                                                                                cleanWs(
                                                                                    patterns: [
            ...
            parallel{
                ...
                stage('Platform Wheels: Mac'){
                    ...
                    steps{
                        script{
                            if(params.INCLUDE_MACOS != true){
                                Utils.markStageSkippedForConditional('Platform Wheels: Mac')
                            }
                            parallel(
                                [failFast: true] +
                                                                ...
                                                                try{
                                                                    try{
                                                                        ...
                                                                        withEnv([
                                                                            ...
                                                                            dockerImage.inside("--label=purpose=ci --label \"absoluteUrl=${currentBuild.absoluteUrl}\" --label \"JOB_NAME=${env.JOB_NAME}\" --label \"BUILD_NUMBER=${currentBuild.number}\" --tmpfs /tmp:exec --tmpfs /.local/share:exec --tmpfs /.local/bin:exec"){
                                                                                withEnv(["UV_CONFIG_FILE=${createUnixUvConfig()}",]){
                                                                                    sh(label: 'Installing required Python version if not already installed', script: "uv python find cpython-${pythonVersion} --quiet 2>/dev/null || uv python install cpython-${pythonVersion}")
                                                                                    unstash 'python sdist'
                                                                                    unstash 'SAMPLE_FILES'
                                                                                    findFiles(glob: 'dist/*.tar.gz').each{
                                                                                        ...
                                                                                        retry(2){
                                                                                            ...
                                                                                            withEnv([(attempt == 1) ? "UV_OFFLINE=1" : 'UV_OFFLINE=0']){
                                                                                                sh(
                                                                                                    label: "Running Tox: ${(attempt == 1) ? "Offline" : 'Online'}",
                                                                                                    script: "uv run --frozen --python-preference system --only-group=tox-uv tox run --installpkg ${it.path} --workdir ./.tox -e py${pythonVersion.replace('.', '')} -vv"
                                                                                                    )
                                                                                            }
                                                                                        }
                                                                                    }
